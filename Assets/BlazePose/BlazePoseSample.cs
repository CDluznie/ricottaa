...
public sealed class BlazePoseSample : MonoBehaviour
{
    ...
    private float[] arrayAngElbow = {      0.0f,  0.0f, 90.0f, 0.0f };
    private float[] arrayAngShoulder = { 180.0f, 90.0f, 90.0f, 0.0f };
    private float thresholdElbow = 30;
    private float thresholdShoulder = 30;
    ...
    void Start()
    {
        ...
        avatars[indexPose].SetActive(true);

        infoText.text = "Starting";


        // Init camer.setActive(true);a
    ...
    void Update()
    {
        ...
        if (landmarkResult != null && landmarkResult.score > 0.2f)
        {
            ...
            var elbowL4 = landmarkResult.joints[PoseLandmarks.LEFT_ELBOW];
            var wristL4 = landmarkResult.joints[PoseLandmarks.LEFT_WRIST];
            var shoulderL = new Vector3(shoulderL4.x, shoulderL4.y);
            var elbowL = new Vector3(elbowL4.x, elbowL4.y);
            var wristL = new Vector3(wristL4.x, wristL4.y);

            var shoulderR4 = landmarkResult.joints[PoseLandmarks.RIGHT_SHOULDER];
            var elbowR4 = landmarkResult.joints[PoseLandmarks.RIGHT_ELBOW];
            var wristR4 = landmarkResult.joints[PoseLandmarks.RIGHT_WRIST];
            var shoulderR = new Vector3(shoulderR4.x, shoulderR4.y);
            var elbowR = new Vector3(elbowR4.x, elbowR4.y);
            var wristR = new Vector3(wristR4.x, wristR4.y);

            var PerpendicularFloor = Vector3.up;
            float angElbowL = Vector3.Angle(shoulderL - elbowL, elbowL - wristL);
            float angShoulderL = Vector3.Angle(elbowL - shoulderL, PerpendicularFloor);
            float angElbowR = Vector3.Angle(shoulderR - elbowR, elbowR - wristR);
            float angShoulderR = Vector3.Angle(elbowR - shoulderR, PerpendicularFloor);

            var renderer = avatars[indexPose].GetComponent<Renderer>();
            if ((Mathf.Abs(angElbowL - arrayAngElbow[indexPose]) < thresholdElbow) && (Mathf.Abs(angElbowR - arrayAngElbow[indexPose]) < thresholdElbow) && (Mathf.Abs(angShoulderL - arrayAngShoulder[indexPose]) < thresholdShoulder) && (Mathf.Abs(angShoulderR - arrayAngShoulder[indexPose]) < thresholdShoulder))
            {
                Debug.Log("ricottaaaaaaa");
    ...
    {
        avatars[indexPose].SetActive(false);
        indexPose = (indexPose + 1) % avatars.Length;
        avatars[indexPose].SetActive(true);
    }
